...
public class Line implements Comparable<Line>, Iterable<Point> {
   ...
   /**
   *Validates the line is collinear by checking the slope between the 
   *initial, final, and unknown point.
   */
   private boolean isCollinear(Point unknown) {
      Point initial = line.first();
      Point finish = line.last();
         if(initial.slopeTo(finish) == initial.slopeTo(unknown)) {
            return true;
         }
         return false;
   }
